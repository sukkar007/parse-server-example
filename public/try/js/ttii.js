...
// متغير للتحقق من جاهزية البيانات
var isPlayerInfoReady = false;
 ...
 * يتم استدعاؤها تلقائياً عند حقن البيانات من Flutter
 */
window.onFlamingoPlayerInfo = function(playerInfo) {
    console.log('=== Received player info from Flutter ===');
    console.log('Raw playerInfo:', playerInfo);
    
    var data;
    try {
        data = typeof playerInfo === 'string' ? JSON.parse(playerInfo) : playerInfo;
    } catch (e) {
    ...
    console.log('Updated info object:', info);
    
    isPlayerInfoReady = true;
...
$(document).ready(function() {
    ...
    // التحقق من وجود معلومات اللاعب المحقونة مسبقاً
    if (window.flamingoPlayerInfo) {
        console.log('Found pre-injected flamingoPlayerInfo:', window.flamingoPlayerInfo);
        window.onFlamingoPlayerInfo(window.flamingoPlayerInfo);
    } else {
        console.log('No pre-injected flamingoPlayerInfo found, waiting...');
        // انتظار البيانات لمدة قصيرة ثم البدء على أي حال
        setTimeout(function() {
            if (!initCalled) {
                console.log('Timeout: Starting without player info');
...
function sureClick(choice, index) {
    ...
    console.log('Placing bet - choice:', choice, 'gold:', currentGold);
...
function getInfo(_round, isChoice) {
    ...
    console.log("Getting game info...", params);
...
// =================== Debug: طباعة حالة البيانات ===================
function debugInfo() {
    console.log('=== DEBUG INFO ===');
    console.log('isPlayerInfoReady:', isPlayerInfoReady);
    console.log('initCalled:', initCalled);
    console.log('info:', info);
    console.log('flamingoPlayerInfo:', window.flamingoPlayerInfo);
    console.log('FlamingoApp available:', !!(window.FlamingoApp && window.FlamingoApp.postMessage));
    console.log('==================');
}

// تشغيل debug بعد ثانيتين
